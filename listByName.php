...
if (isset($_GET['var'])) {
    list($email, $name, $access) = explode(":", $_GET['var']);

    if (strlen($name) == 0) {
        $dailyArr = getDaily($email, $access, $db_connection);
        printList($dailyArr);
    } else {
        $dailyArr = getList($email, $name, $access, $db_connection);
        printList($dailyArr);
    }
}
...
function getList($email, $name, $access, $db_connection)
{
    //Array that will be returned
    $dailyArr = array();
    //Counter
    ...
    if ($access) {
        $stmt = mysqli_prepare($db_connection, "SELECT c.fName, c.lName, d.date, d.activity, d.bFast, d.lunch, d.temperature, d.cID FROM day_detail d INNER JOIN child c ON c.cID = d.cID WHERE c.fName LIKE ? OR c.lName LIKE ?;");
        mysqli_stmt_bind_param($stmt, 'ss',  $name, $name);
    } else {
        $stmt = mysqli_prepare($db_connection, "SELECT c.fName, c.lName, d.date, d.activity, d.bFast, d.lunch, d.temperature, d.cID, c.pID  FROM day_detail d INNER JOIN child c ON c.cID = d.cID WHERE c.fName LIKE ? OR c.lName LIKE ? HAVING c.pID=?;");
        mysqli_stmt_bind_param($stmt, 'sss',  $name, $name, $email);
    }
